...
input_error_msg = (
    "Input must contain following named elements: 'ref', 'in1', and 'in2'."
)
assert isinstance(
    inputs["ref"], str
), "Must be path to reference fasta (e.g. contigs.fa)."
...
# Run commands.
shell("bbwrap.sh {input_flags} out={snakemake.output.aln} {options}")
shell("pileup.sh in={snakemake.output.aln} out={snakemake.output.cov}")
