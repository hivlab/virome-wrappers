...
taxranks = ["SUBSPECIES", "SPECIES", "GENUS", "FAMILY", "ORDER", "CLASS", "PHYLUM", "KINGDOM", "SUPERKINGDOM", "DOMAIN", "LIFE"]
taxranks = [i.lower() for i in taxranks]
...
def assign_consensus_taxonomy(blast_df, pp_sway, id, out_prefix):
    ...
    for query, hits in by_query:
        ...
        pd.DataFrame(consensus_taxonomy).to_csv("{}_{}.csv".format(out_prefix, id), index = False)
